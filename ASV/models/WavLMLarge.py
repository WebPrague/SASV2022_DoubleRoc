...
def MainModel(model_file_path):
    model = init_model('wavlm_large', model_file_path)
    return model

if __name__ == '__main__':
    import numpy as np
    # input_np = np.random.randn(1, 5*16000).astype(np.float32)
    # np.save('input_np.npy', input_np)
    inp = torch.from_numpy(np.load('input_np.npy')).float()

    model = MainModel('./weights/wavlm_large_finetune.model')
    model.eval()
    with torch.no_grad():
        outp = model(inp)
        print(outp)
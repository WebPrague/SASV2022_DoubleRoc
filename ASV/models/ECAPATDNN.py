...
def resume_spk_model(model_file_path, network):
    ...
    drop_keys = filter(lambda key: not key.startswith('speaker_encoder.'), list(model_dict.keys()))
    use_keys = filter(lambda key: key.startswith('speaker_encoder.'), list(model_dict.keys()))
    for drop_key in drop_keys:
        model_dict.pop(drop_key)
    for use_key in use_keys:
        new_key = use_key.replace('speaker_encoder.', '')
        model_dict[new_key] = model_dict[use_key]
        model_dict.pop(use_key)
...
if __name__ == '__main__':
    model = MainModel('weights/ecapatdnn.model')
    inp = torch.randn(3, 16000*3)
    outp = model(inp)
    print(outp.shape)
...
def resume_spk_model(model_file_path, network):
    ...
    drop_keys = filter(lambda key: not key.startswith('__S__.'), list(model_dict.keys()))
    use_keys = filter(lambda key: key.startswith('__S__.'), list(model_dict.keys()))
    for drop_key in drop_keys:
        model_dict.pop(drop_key)
    for use_key in use_keys:
        new_key = use_key.replace('__S__.', '')
...
def MainModel(model_file_path, nOut=512):
    kwargs = {'log_input': True, 'encoder_type': 'ASP', 'n_mels': 64, }
    # Number of filters
